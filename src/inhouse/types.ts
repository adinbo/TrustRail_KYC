...
export interface InHouseTamperResult {
  passed: boolean;
  tamperScore: number; // 0.0 (clean) to 1.0 (heavily tampered)
  checks: {
...
export interface InHouseQualityResult {
  passed: boolean;
  sharpnessScore: number; // 0.0 to 1.0 (Laplacian clarity)
  illuminationScore: number; // 0.0 (dark) to 1.0 (bright)
  glareScore: number; // 0.0 (none) to 1.0 (heavy glare)
  isBlurry: boolean;
  isDark: boolean;
  hasGlare: boolean;
  flags: string[];
