...
export class InHouseOcrEngine {
    ...
    async extractDocumentData(input) {
        ...
        // Parse names
        if (input.firstName)
            extractedFields.firstName = input.firstName.trim();
        if (input.lastName)
            extractedFields.lastName = input.lastName.trim();
        if (input.firstName && input.lastName) {
            extractedFields.fullName = `${input.firstName.trim()} ${input.lastName.trim()}`;
        }
        // Parse Dates
        if (input.dateOfBirth)
            extractedFields.dateOfBirth = input.dateOfBirth.trim();
        if (input.expiryDate)
            extractedFields.expiryDate = input.expiryDate.trim();
